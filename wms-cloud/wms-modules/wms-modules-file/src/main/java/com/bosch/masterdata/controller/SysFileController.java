...
//            }
    ...
    @ApiOperation("SPDN导入")
    @PostMapping(value = "/SPDNFileImport")
    public <T> R<List<T>> SPDNFileImport(@RequestPart(value = "file") MultipartFile file, @RequestParam(value =
            "className") String className) throws Exception {
        try {
            Class<?> TClass = Class.forName("com.bosch.product.api.domain.dto." + className);
            List<T> read = EasyExcelUtil.readNoValid(file.getInputStream(), TClass,className);
    ...
    @ApiOperation("SPDN导入")
    @PostMapping(value = "/SUDNFileImport")
    public <T> R<List<T>> SUDNFileImport(@RequestPart(value = "file") MultipartFile file, @RequestParam(value =
            "className") String className) throws Exception {
        try {
            Class<?> TClass = Class.forName("com.bosch.product.api.domain.dto." + className);
            List<T> read = EasyExcelUtil.readNoValid(file.getInputStream(), TClass,className);
