...
public class ExcelUtil<T> {
    ...
    public void exportExcel(HttpServletResponse response, List<T> list, String sheetName, List<ExportUtilParam> lst) {
        exportExcel(response, list, sheetName, StringUtils.EMPTY, lst);
    }
    ...
    public void exportExcel(HttpServletResponse response, List<T> list, String sheetName, String title, List<ExportUtilParam> lst) {
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        this.init(list, sheetName, title, Type.EXPORT);
        exportExcel(response, lst);
    }
    ...
    public void exportExcel(HttpServletResponse response, List<ExportUtilParam> lst) {
        try {
            writeSheet(lst);
            wb.write(response.getOutputStream());
        } catch (Exception e) {
     ...
     * 创建写入数据到Sheet
     */
    public void writeSheet(List<ExportUtilParam> lst) {
        // 取出一共有多少个sheet.
    ...
    public void fillExcelData(int index, Row row, List<ExportUtilParam> lst) {
        ...
        if (list.size() > 0 && lst != null) {
            ...
            Row titleRow = this.sheet.getRow(titleRowIndex - 1);
            ...
            for (ExportUtilParam param : lst) {
                for (Integer i : colIndex) {
                    CellRangeAddress region = new CellRangeAddress(param.getStartRow(), param.getEndRow(), i, i);
