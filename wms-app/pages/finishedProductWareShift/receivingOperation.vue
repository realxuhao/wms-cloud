...
<script>
	...
	export default {
		name: 'transhipmentOrderProductlIn',
		components: {
			Message,
			MyRadio
		},
		...
		onLoad(options) {
			this.barCode = options.barCode.replace(/\r|\n/gi, '');;
		...
		methods: {
			...
			async handleSubmit() {
				try {
					...
					await this.$store.dispatch('finishedProduct/mainReceiveConfirm', {
						sscc
					});
					...
					uni.navigateTo({
						url: '/pages/finishedProductWareShift/receivingScan'
					});
