<template>
	<my-page nav-title="打包" :noReturn="true" @returnCallBack="handleReturn">
					...
					<uni-list v-show="currentTaskBarCodeList.length">
						<uni-list-item v-for="(item, index) in currentTaskBarCodeList" :key="item.sscc" ellipsis="1">
							...
							<template v-slot:footer>
								<uni-icons @click="handleDelete(index)" custom-prefix="custom-icon" type="closeempty"
									size="18" color="#541b86"></uni-icons>
							</template>
						</uni-list-item>
					</uni-list>
		...
		<uni-popup ref="alertDialog" type="dialog">
			<uni-popup-dialog type="error" cancelText="关闭" confirmText="清空打包任务" title="通知" content="扫描标签次数未满足要求,请检查!"
	...
	export default {
		...
		methods: {
			...
			onSubmitCheck() {
				...
				if (this.allScanProdOrderList > this.palletQuantityCount || this.allScanProdOrderList < this
					.palletQuantityCount) {
					throw ({
						message: `扫描标签次数未满足要求,请检查`
					});
				}
			...
			handleDelete(index) {
				...
				this.allScanProdOrderList.splice(index, 1)
			},
			async handleClean() {
				try {
					const deleteParam = {
