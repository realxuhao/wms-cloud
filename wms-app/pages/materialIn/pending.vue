...
export default {
  ...
  methods: {
	  async getPendingMaterialList(){
		const options = {pageSize:this.pageSize,pageNum:this.pageNum,status:0}
		const {rows,total} = await this.$store.dispatch('materialIn/getPendingMaterialList',options)
		return {rows,total}
	  },
	  async handleRefresh(){
	  	await this.loadData()
	  	...
	  	try{
	  		...
	  		if (length < this.total) {
				this.bottomTips = 'loading'
	  			this.pageNum +=1
	  			const {rows} = await this.getPendingMaterialList()
	...
	async loadData(){
		try{
			this.pageNum = 1
			const {rows,total} = await this.getPendingMaterialList()
			this.list = rows
			this.total = total
		}catch(e){
			this.$refs.message.error(e.message)
