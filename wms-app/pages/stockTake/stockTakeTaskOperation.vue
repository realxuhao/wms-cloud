		...
		<view class="main" slot="page-main">
			<view class="content">
				<uni-forms :label-width="80" ref="binInForm" :rules="formRules" :modelValue="form" label-position="left">
					...
					<uni-forms-item label="盘点类型"><uni-tag type="primary" :text="materialInfo.type === 1 ? '盲盘' : '明盘'"></uni-tag></uni-forms-item>
					...
					<uni-forms-item label="库存量">{{ materialInfo.type === 1 ? '***' : materialInfo.stockQuantity }}</uni-forms-item>
					<uni-forms-item v-if="materialInfo.type === 0" label="是否一致" required><uni-data-checkbox v-model="form.isDiff" :localdata="diffList" /></uni-forms-item>
					<uni-forms-item v-if="materialInfo.type === 1 || !form.isDiff" label="盘点量" name="pdaTakeQuantity" required>
