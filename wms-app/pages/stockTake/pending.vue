<template>
	<view class="content">
		...
		>
			<!-- 插入自己的数据-->
			<view class="card" @click="$emit('on-click', item)" v-for="item in list" :key="item.id">
				<view class="card-header">
					<text class="material-name">{{ item.taskNo }}</text>
					<text class="status">未盘点</text>
				</view>
				<view class="card-text m-b-4">下发时间：{{ item.issueTime }}</view>
			</view>
