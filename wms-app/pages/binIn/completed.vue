	...
	<view class="content">
		...
		>
			...
			<view class="card" v-for="item in list" :key="item.id">
				...
				<view class="card-text m-b-4">目标库位：{{ item.recommendBinCode }}</view>
...
export default {
	...
	methods: {
		async getBinInHistoryList() {
			...
			const { rows, total } = await this.$store.dispatch('binIn/getBinInHistoryList', options);
			return { rows, total };
		},
		async loadData() {
			try {
				this.pageNum = 1;
				const { rows, total } = await this.getBinInHistoryList();
				this.list = rows;
				this.total = total;
			} catch (e) {
		...
		async handleLoadMore() {
			try {
				...
				if (length < this.total) {
					...
					this.pageNum += 1;
					const { rows } = await this.getBinInHistoryList();
