				...
				<uni-forms :label-width="80" ref="binInForm" :rules="binInFormRules" :modelValue="binInForm"
					...
					<uni-forms-item v-if="!binInInfo.palletCode" label="托盘编码" name="palletTypeCode" required>
						<view class="flex flex-ai">
							<view class="custom-input" :class="editFieldName==='binInForm.palletTypeCode'?'focus':''"
								@click="()=>handleSetEditFieldName('binInForm.palletTypeCode')">
								<text
									:class="!binInForm.palletTypeCode?'placeholder-text':''">{{binInForm.palletTypeCode||'请扫描托盘编码'}}</text>
							</view>
						</view>
					</uni-forms-item>
					
					<uni-forms-item label="目标区域" name="recommendBinCode" v-if="manualTransInfo.type===1" required>
						<view class="custom-input" :class="editFieldName==='binInForm.recommendBinCode'?'focus':''"
							@click="()=>handleSetEditFieldName('binInForm.recommendBinCode')">
							<text
								:class="!binInForm.recommendBinCode?'placeholder-text':''">{{binInForm.recommendBinCode||'请扫描目标库位'}}</text>
						</view>
					</uni-forms-item>
					...
					<o-btn block class="submit-btn primary-button" :loading="submitLoading" @click="handlePostBinIn">提交
					</o-btn>
				</uni-forms>
			...
			<uni-popup ref="popup" :is-mask-click="false">
				<view class="result-content">
					<view class="result-status">
						<uni-icons custom-prefix="iconfont" class="success-color" type="icon-chenggong" size="32">
						</uni-icons>
