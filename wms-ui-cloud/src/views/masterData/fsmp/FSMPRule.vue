  ...
  <div class="wrapper">
    ...
    <div class="table-content">
        ...
        <a-row :gutter="16">
          ...
          <a-col span="4">
            <span class="table-page-search-submitButtons" >
              <a-button v-hasPermi="['fsmp:sample:rule:query']" type="primary" @click="handleSearch" :loading="searchLoading"><a-icon type="search" />查询</a-button>
      ...
      <div class="action-content">
        <a-button v-hasPermi="['fsmp:sample:rule:add']" type="primary" class="m-r-8" icon="plus" @click="handleAdd">
          新建
        </a-button>
        <a-tooltip placement="right">
          <template slot="title">
            <a v-hasPermi="['fsmp:sample:rule:download']" style="color:#fff" @click="handleDownloadTemplate"><a-icon type="arrow-down" />下载模板</a>
          </template>
          <a-upload
            :file-list="[]"
            name="file"
            ...
            @change="handleUpload"
          >
            <a-button v-hasPermi="['fsmp:sample:rule:import']" :loading="uploadLoading" type="primary" icon="upload" >
              导入
            </a-button>
          </a-upload>
        </a-tooltip>
      ...
      >
        ...
        <template slot="action" slot-scope="text, record">
          <div class="action-con">
            <a v-hasPermi="['fsmp:sample:rule:edit']" class="warning-color" @click="handleEdit(record)"><a-icon class="m-r-4" type="edit" />编辑</a>
            ...
            >
              <a v-hasPermi="['fsmp:sample:rule:delete']" class="danger-color"><a-icon class="m-r-4" type="delete" />删除</a>
            </a-popconfirm>
            <!-- <a
              class="primary-color"
