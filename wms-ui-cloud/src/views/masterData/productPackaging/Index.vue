  ...
  <div class="wrapper">
    ...
    <div class="table-content">
      <a-form layout="inline" class="search-content">
        <a-row :gutter="16">
          <a-col :span="4">
            <a-form-model-item label="成品料号">
              <a-input v-model="queryForm.productNo" placeholder="成品料号" allow-clear/>
            </a-form-model-item>
          </a-col>
          <a-col :span="4">
            <a-form-model-item label="成品名称">
              <a-input v-model="queryForm.productName" placeholder="成品名称" allow-clear/>
            </a-form-model-item>
          </a-col>
          <a-col :span="4">
  ...
  methods: {
    ...
    async loadTableList () {
      try {
        ...
        const { data: { rows, total } } = await this.$store.dispatch('productPackaging/getList', this.queryForm)
        this.list = rows
    ...
    async handleDownloadTemplate () {
      try {
        this.$store.dispatch('file/downloadByFilename', '成品包装量.xlsx')
      } catch (error) {
        this.$message.error(error.message)
      }
    },
    async uploadBatchUpdate (formdata) {
      try {
        await this.$store.dispatch('productPackaging/uploadBatchUpdate', formdata)
        this.loadTableList()
      } catch (error) {
        this.$message.error(error.message)
      } finally {
    ...
    async handleUpload (e) {
      ...
      try {
        ...
        await this.$store.dispatch('productPackaging/upload', formdata)
