  ...
  methods: {
    ...
    async handleSubmit () {
      try {
        ...
        const { data } = await this.$store.dispatch('finishedProductQualityManage/saveEditSSCCStatusList', this.editSSCCList)
    ...
    async loadTableList () {
      try {
        ...
        } = await this.$store.dispatch('finishedProductQualityManage/getQualityList', options)
        this.list = rows
    ...
    async onSubmit (record) {
      ...
      try {
        ...
        await this.$store.dispatch('finishedProductQualityManage/updateIqcQuality', options)
